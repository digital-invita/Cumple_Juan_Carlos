    ...
    const INVITATION_CONFIG =
{
  ...
  "story": {
    "title": "Mensaje de misión",
    "paragraph1": "Un año de aventuras, risas y mucho amor merece una gran celebración. ¡Esperamos contar con tu presencia!",
    "paragraph2": "",
    "showImage": false
  },
  "family": {
  ...
  "timeline": {
    "items": [
      {
        "time": "14:00 hrs",
        "title": "Llegada de tripulación",
        "desc": "Registro de invitados y bienvenida."
      },
      {
        "time": "15:00 hrs",
        "title": "Comida espacial",
        "desc": "Compartimos la mesa para celebrar."
      },
      {
        "time": "16:30 hrs",
        "title": "Pastel y fotos",
        "desc": "Momento especial para Juan Carlos."
      },
      {
        "time": "18:00 hrs",
        "title": "Cierre de misión",
        "desc": "Gracias por acompañarnos en esta aventura."
      }
    ]
  },
  "countdown": {
    "title": "T-minus para despegar",
    "intro": "El Comando Estelar prepara la misión de cumpleaños."
  },
  "dressCode": {
    "title": "Dress Code",
    "value": "Cómodo para despegar",
    "note": "Puedes venir con detalles verdes, blancos o morados si quieres sumarte a la misión."
  },
  "gifts": {
    "title": "Detalles para el comandante",
    "intro": "Tu presencia es el regalo más valioso. Si deseas tener un detalle para Juan Carlos, lo recibiremos con mucho cariño.",
    "option1Title": "Sobre de misión",
    "option1Copy": "Si lo prefieres, puedes acompañarnos con un sobre el día del evento. Será recibido con mucha gratitud.",
    "showOption1": true,
    "option2Title": "Regalo sorpresa",
    "option2Copy": "También puedes sorprender a Juan Carlos con un detalle elegido por ti. Lo importante será el cariño con el que venga.",
    "showOption2": true,
    "linksIntro": "Si prefieres elegir un detalle en línea, puedes apoyarte en estas opciones:",
    "showLinks": false,
    "links": [
      {
  ...
  "album": {
    "title": "Álbum digital",
    "copy": "Comparte las mejores fotos de esta misión y conserva en un solo lugar los recuerdos más especiales.",
  ...
  "closing": {
    "note": "Hasta el infinito y más allá: gracias por acompañar a Juan Carlos en su primera gran misión.\n"
  },
  "brand": {
    ...
    const STORAGE_KEY = "digital-invita-editor-config-buzz-juan-carlos-v3";
    const BASE_STORAGE_KEY = "digital-invita-editor-base-buzz-juan-carlos-v3";
    ...
    function cssFontValue(label, fallback) {
      ...
      if (label === "Orbitron") return "\"Orbitron\", sans-serif";
      return fallback;
    }
    ...
    function applyInvitationConfig() {
      ...
      if (story2) {
        ...
        story2.style.display = INVITATION_CONFIG.story.paragraph2.trim() ? "" : "none";
      }
    ...
    function sendConfirmation(attending) {
      ...
      if (attending) {
        ...
        message = [
          ...
          "",
          `Con mucha alegría confirmo mi presencia para la misión ${eventCopy.celebrationText}.`,
          "",
          `Nombre: ${name}`,
          ...(INVITATION_CONFIG.rsvp.showGuestCount !== false ? [count] : []),
          "",
          "Será un honor acompañarlos en este despegue tan especial."
        ].join("\n");
      } else {
        message = [
          ...
          "pero le deseo a Juan Carlos una misión maravillosa y llena de felicidad."
        ].join("\n");
      }
    ...
    function initParticles() {
      ...
      function createParticle(kind = "star") {
        const isComet = kind === "comet";
        return {
          x: Math.random() * width,
          y: Math.random() * height,
          r: isComet ? Math.random() * 1.2 + 0.7 : Math.random() * 1.8 + 0.6,
          vx: isComet ? -(Math.random() * 1.1 + 0.8) : (Math.random() - 0.5) * 0.22,
          vy: isComet ? (Math.random() * 0.72 + 0.22) : (Math.random() - 0.5) * 0.22,
          alpha: isComet ? Math.random() * 0.35 + 0.38 : Math.random() * 0.35 + 0.15,
          kind,
          twinkle: Math.random() * Math.PI * 2,
          length: isComet ? Math.random() * 54 + 38 : 0
        };
      }
      ...
      function resize() {
        ...
        const total = IS_MOBILE_DEVICE
          ...
          : (lowPower
            ? (width < 768 ? 16 : 26)
            : (width < 768 ? 44 : 82));
        const cometTotal = lowPower ? 1 : (IS_MOBILE_DEVICE ? 2 : 5);
        particles = [
          ...Array.from({ length: total }, () => createParticle("star")),
          ...Array.from({ length: cometTotal }, () => createParticle("comet"))
        ];
      }
      ...
      function animate() {
        ...
        particles.forEach((particle) => {
          ...
          particle.twinkle += particle.kind === "comet" ? 0.02 : 0.045;

          if (particle.kind === "comet") {
            if (particle.x < -particle.length || particle.y > height + particle.length) {
              particle.x = width + Math.random() * width * 0.35;
          ...
          ctx.beginPath();
          if (particle.kind === "comet") {
            const gradient = ctx.createLinearGradient(particle.x, particle.y, particle.x + particle.length, particle.y - particle.length * 0.34);
            gradient.addColorStop(0, `rgba(${particleRgb}, ${alpha})`);
            gradient.addColorStop(1, `rgba(${particleRgb}, 0)`);
            ctx.strokeStyle = gradient;
            ctx.lineWidth = particle.r;
            ctx.moveTo(particle.x, particle.y);
            ctx.lineTo(particle.x + particle.length, particle.y - particle.length * 0.34);
            ctx.stroke();
          } else {
    ...
    openBtn.addEventListener("click", () => {
      ...
      window.setTimeout(triggerWarpFlash, 280);
      window.setTimeout(() => {
